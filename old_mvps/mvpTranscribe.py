...
speech_debounce_delay = 2.0  # Wait for 2 seconds of no speech before processing

# Audio recording variables
audio_frames = []
p = pyaudio.PyAudio()
stream = None
stream_active = False  # New flag to track stream state
...
def start_audio_recording():
    global stream, audio_frames, is_recording, stream_active
    if not is_recording:
        try:
            audio_frames = []
...
def stop_audio_recording():
    global stream, is_recording, stream_active
        ...
        with tempfile.NamedTemporaryFile(suffix=".wav", delete=False) as temp_wav:
            ...
            with sr.AudioFile(temp_wav.name) as source:
                ...
                try:
                    ...
                    # Update last speech time for debouncing
                    last_speech_time = time.time()
                    # Add to queue for processing
                    audio_queue.put(text)
                    print(f"Added transcribed text to queue: {text}")
...
def check_for_speech_enhancement(current_canvas):
    global last_speech_time, is_processing
    
    current_time = time.time()
    
    # Don't process if already processing or if no speech in queue
    if is_processing or audio_queue.empty():
        return False
        
    # Check if enough time has passed since last speech
    if (current_time - last_speech_time) > speech_debounce_delay:
        # Get the next transcribed text from the queue
        prompt = audio_queue.get()
        print(f"Processing speech prompt: {prompt}")
        
        # Create a more detailed prompt that includes the transcribed text
        full_prompt = f"Enhance this drawing based on the following description: {prompt}"
        print(f"Full prompt being sent to Gemini: {full_prompt}")
        
        # Trigger enhancement with the speech prompt
        enhance_drawing_with_gemini(current_canvas, full_prompt)
        return True
...
def audio_recording_thread():
    global audio_frames, is_recording, stream_active, stream
    while True:
        try:
            if is_recording and stream_active and stream is not None:
                data = stream.read(CHUNK, exception_on_overflow=False)
                audio_frames.append(data)
                # Check for silence
                if max(abs(int.from_bytes(data[i:i+2], byteorder='little', signed=True)) 
                      for i in range(0, len(data), 2)) < SILENCE_THRESHOLD:
                    # If we detect enough silence, process the audio
                    if len(audio_frames) > int(RATE / CHUNK * SILENCE_DURATION):
                        process_audio()
                        audio_frames = []
        except Exception as e:
            if "Stream closed" in str(e):
                # Stream was closed, update state
                stream_active = False
                is_recording = False
                if stream:
                    stream.close()
...
while True:
    ...
    if not was_drawing and not is_processing and (int(time.time() * 5) % 5 == 0):
        ...
        check_for_speech_enhancement(canvas.copy())
    ...
    cv2.putText(combined_img, recording_status, (10, 150), cv2.FONT_HERSHEY_SIMPLEX, 0.7,
    ...
    if last_enhanced_canvas is not None and auto_enhance_enabled:
        ...
        cv2.putText(combined_img, f"Similarity: {similarity:.2f}", (10, 190), 
    ...
    cv2.putText(combined_img, "t=Adjust sensitivity | s=Save | m=Toggle recording | q=Quit", 
    ...
    elif key == ord('m'):  # m to toggle audio recording
        if is_recording:
            stop_audio_recording()
            recording_status = "RECORDING: OFF"
        else:
            start_audio_recording()
            recording_status = "RECORDING: ON"
        print(f"Audio recording: {'started' if is_recording else 'stopped'}")
