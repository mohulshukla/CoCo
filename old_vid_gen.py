...
class StoryVideoGenerator:
    ...
    def create_audio(self, text, output_path):
        """Generate audio narration using gTTS (much faster than pyttsx3)."""
        try:
            logging.info("Starting audio generation...")
            start_time = time.time()
            
            # Use gTTS for faster audio generation
    ...
    def generate_video(self):
        ...
        try:
            ...
            # Combine all text into one narration
            full_narration = f"{story_data['title']}. {story_data['story']} "
            for i, desc in enumerate(story_data["scene_descriptions"], 1):
                full_narration += f"Scene {i}: {desc} "
